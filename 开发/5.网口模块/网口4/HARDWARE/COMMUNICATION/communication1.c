#include "uart.h"
#include "delay.h"
#include "time.h" 
#include "equipment.h"
#include "communication1.h"
#include "show1.h"

//uart1下位机通信
ModbusSend_typedef ModbusTx;					//Modbus发送
ModbusReceive_typedef ModbusRx;			//Modbus接收
u8 UpdateNum = 0;//串口1中断  判断功能码
u8 DataReceive[250];
u8 DataReceive6[20];


//模式控制
#define RS485_TX_EN		PAout(1)	//485模式控制.0,接收;1,发送.

u8 ModbusEquipmentExist[8];//设备存在数组，0不存在，1存在

/// CRC 高位字节值表 ///    
const u8  auchCRCHi[] = {    
    
} ;    
/// CRC低位字节值表///    
const u8  auchCRCLo[] = {    
    
} ;   
   

//初始化modbus
void ModbusInit(void);
//Modbus发送指令
void ModbusSendOrder(void);


//通信初始化
void communication1_init(void)
{
	uart2_init(36,9600);	//串口2初始化
	//uart2_init(72,19200);	//串口2初始化
	
//	//为485通信添加
	RCC->APB2ENR |=1<<2;//使能端口PORTA口时钟
	GPIOA->CRL &=0xFFFFFF0F;
	GPIOA->CRL |=0x00000030;//PORTA1推挽输出
	GPIOA->ODR &=~(1<<1);//GPIOA1默认输出为0
	
	TIM2_Int_Init(99,7199);//10毫秒
	
	ModbusInit();
}


//初始化modbus
void ModbusInit(void)
{
	u16 i;
	for(i=0;i<MoubusReceArrLength;i++)
	{
		ModbusTx.Txbuf[i]=0;
		ModbusRx.Rxbuf[i]=0;
	}
	for(i=0;i<8;i++)
	{
		ModbusEquipmentExist[i]=0;
	}
	
	ModbusTx.EquipmentNum=0;
	ModbusTx.ModbusOrder=0;
	ModbusTx.StartAddress=0;
	ModbusTx.RegisterNum=0;
	ModbusTx.Txlen=0;
	ModbusTx.CrcData=0;
	
	ModbusRx.EquipmentNum=0;
	ModbusRx.ModbusOrder=0;	
	ModbusRx.StartAddress=0;	
	ModbusRx.RegisterNum=0;
	ModbusRx.Rxlen=0;
	ModbusRx.ModbusReceDoneNum=0;
}

u8 ModbusReceiveOrderCheck(void)
{
	if(ModbusRx.Rxbuf[1]==6)
	{
		return 0xFF;
	}
	return 0x00;
}
u8 ModbusReceiveOrderCheck3(void)
{
	if(ModbusRx.Rxbuf[1]==3)
	{
		return 0xFF;
	}
	return 0x00;
}
u8 ModbusReceiveOrderCheck16(void)
{
	if(ModbusRx.Rxbuf[1]==16)
	{
		return 0xFF;
	}
	return 0x00;
}



//Modbus发送指令
void ModbusSendOrder(void)
{
    {
			RS485_Send_Data(ModbusTx.Txbuf,ModbusTx.Txlen);		//发送数据
			ModbusRx.Rxlen=0;//准备接收
			ModbusRx.ModbusReceDoneNum=0;
		}
}

//Modbus发送指令
void ModbusSendOrder1(void)
{
    {
			RS485_Send_Data(ModbusTx.Txbuf,ModbusTx.Txlen);		//发送数据
			ModbusRx.Rxlen=0;//准备接收
			ModbusRx.ModbusReceDoneNum=0;
		}
}


//485接收
void RS485_Read(void)
{
	RS485_TX_EN=0;
}
//485发送
void RS485_Send(void)
{
	RS485_TX_EN=1;
}
//RS485发送len个字节.
//buf:发送区首地址
//len:发送的字节数(为了和本代码的接收匹配,这里建议不要超过64个字节)
void RS485_Send_Data(u8 *buf,u8 len)
{
	u8 t;
	RS485_TX_EN=1;			//设置为发送模式
  for(t=0;t<len;t++)		//循环发送数据
	{
		while((USART2->SR&0X40)==0);//等待发送结束		  
		USART2->DR=buf[t];
	}	 
	while((USART2->SR&0X40)==0);//等待发送结束	  
	RS485_TX_EN=0;				//设置为接收模式	
}

//串口1中断函数
void USART2_IRQHandler(void)
{
	u8 res;
	if(USART2->SR&(1<<5))			//接收到数据
	{	 
		res=USART2->DR; 
		if(ModbusRx.Rxlen<MoubusReceArrLength) 
		{
			ModbusRx.Rxbuf[ModbusRx.Rxlen]=res;
			DataReceive[ModbusRx.Rxlen] = res;
			ModbusRx.ModbusReceDoneNum=1;		
			ModbusRx.Rxlen++;
//			if(ModbusRx.Rxlen-1 == 1){
//				if(ModbusRx.Rxbuf[ModbusRx.Rxlen-1] == 3){
//						UpdateNum = 3;//串口1中断  判断功能码
//				}if(ModbusRx.Rxbuf[ModbusRx.Rxlen-1] == 6){
//						UpdateNum = 6;
//				}if(ModbusRx.Rxbuf[ModbusRx.Rxlen-1] == 16){
//						UpdateNum = 16;
//				}
//			}
//			if(UpdateNum == 6 || UpdateNum == 16){
//				DataReceive6[ModbusRx.Rxlen-1] = res;
//			}if(UpdateNum == 3){
//				DataReceive[ModbusRx.Rxlen-1] = res;
//			}
		} 

	}
}
//CRC校验码，高字节在前，低字节在后
u16 crc16(u8 *puchMsg, u16 usDataLen)    
{    
    u8 uchCRCHi = 0xFF ;						/// 高CRC字节初始化 ///    
    u8 uchCRCLo = 0xFF ;						/// 低CRC 字节初始化 ///    
    u8 uIndex ;											/// CRC循环中的索引 ///    
    while (usDataLen--)							/// 传输消息缓冲区 ///    
    {    
        uIndex = uchCRCHi^(*puchMsg++) ;			/// 计算CRC ///    
        uchCRCHi = uchCRCLo^auchCRCHi[uIndex];    
        uchCRCLo = auchCRCLo[uIndex] ;    
    }    
    return (uchCRCHi<<8 | uchCRCLo) ;    
}


//Modbus事件处理函数
void Modbus_Event_1(void){

	//判断是否接收Modbus信息
	if(ModbusRx.Rxlen == 0){
		
			return;
	}
	//判断两条信息的间隔时间
	if(ModbusRx.ModbusReceDoneNum <= 2){
		
			return ;
	}
	
	if(ModbusRx.ModbusReceDoneNum >= 3){
				if(DataReceive[0] == ModbusBroadcastAddress ){
					
					switch(DataReceive[1])		//分析功能码
					{
						case 3:						//读取寄存器的数据
								showData1_1();
							break;
						case 6:					//写入单个寄存器数据
							ModbusFun6_1();
							break;
						
						default:break;
					}
			}
			ModbusRx.Rxlen = 0;
			ModbusRx.ModbusReceDoneNum = 0;
	}
}


 // Modbus 6号功能码函数
 // Modbus 主机写入寄存器值

void ModbusFun6_1(void)	
{

	u16 Regadd;
	u16 Temp;
	//u16 val;
	//得到要读取寄存器的首地址
	Regadd = DataReceive[2]<<8 | DataReceive[3];		//修改寄存器首地址
	
	switch(Regadd){
		
		case 0:
			Temp = DataReceive[4]<<8 | DataReceive[5];
			SetReceiveOrSend(Temp);			//修改接收或者发送
			break;
		case 1:		
			Temp = DataReceive[4]<<8 | DataReceive[5];			
			SetMode(Temp);
			break;
		case 2:		
			Temp = DataReceive[4]<<8 | DataReceive[5];			
			SetLocalGroupNumber(Temp);
			break;
		case 3:		
			Temp = DataReceive[4]<<8 | DataReceive[5];			
			SetLocalAddress(Temp);
			break;
		case 4:		
			Temp = DataReceive[4]<<8 | DataReceive[5];			
			SetTargetGroupNumber(Temp);
			break;
		case 5:		
			Temp = DataReceive[4]<<8 | DataReceive[5];			
			SetDestinationAddress(Temp);
			break;
		case 6:		
			Temp = DataReceive[4]<<8 | DataReceive[5];			
			SetChannelAdd(Temp);
			break;
		case 7:		
			Temp = DataReceive[4]<<8 | DataReceive[5];			
			SetArmChannelAdd(Temp);
			break;
//		case 8:		
//			Temp = DataReceive[4]<<8 | DataReceive[5];			
//			SetTempChannelAdd(Temp);
//			break;
		default:
			break;
	
	}
	
}


//输出数据
void showData1_1(void)
{
	u16 crc;
	ModbusTx.Txbuf[0]=ModbusBroadcastAddress;
	ModbusTx.Txbuf[1]=0x03;
	
	ModbusTx.Txbuf[2]=0x10;
	
	ModbusTx.Txbuf[3]=ReceiveOrSend>>8;
	ModbusTx.Txbuf[4]=ReceiveOrSend;
	ModbusTx.Txbuf[5]=Mode>>8;
	ModbusTx.Txbuf[6]=Mode;
	ModbusTx.Txbuf[7]=LocalGroupNumber>>8;
	ModbusTx.Txbuf[8]=LocalGroupNumber;
	ModbusTx.Txbuf[9]=LocalAddress>>8;
	ModbusTx.Txbuf[10]=LocalAddress;
	ModbusTx.Txbuf[11]=TargetGroupNumber>>8;
	ModbusTx.Txbuf[12]=TargetGroupNumber;
	ModbusTx.Txbuf[13]=DestinationAddress>>8;
	ModbusTx.Txbuf[14]=DestinationAddress;
	ModbusTx.Txbuf[15]=Channel>>8;
	ModbusTx.Txbuf[16]=Channel;
	ModbusTx.Txbuf[17]=ArmChannel>>8;
	ModbusTx.Txbuf[18]=ArmChannel;

	
	crc=crc16(ModbusTx.Txbuf,19);
	ModbusTx.Txbuf[19]=crc>>8;
	ModbusTx.Txbuf[20]=crc;		
	ModbusTx.Txlen=21;	
	ModbusSendOrder();
}
